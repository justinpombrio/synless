...
/// Obtain one by calling `.as_mut()` on a `Tree`.
///
...
impl<'t, 'l> TreeMut<'t, 'l> {
    ...
    /// children is modifed.*
    pub(in tree) fn update(&mut self) {
        let n = self.path.len();
        for gen in 0 .. n + 1 {
    ...
    pub(in tree) fn as_ref<'t2>(&'t2 self) -> TreeRef<'t2, 'l> {
        TreeRef::new(&self.root, self.path.clone())
    }

    /// Replace this tree in-place. Returns the old tree.
    pub(in tree) fn replace(&mut self, tree: Tree<'l>) -> Tree<'l> {
        mem::replace(&mut self.tree_mut(), tree)
    }

    // Local Info //

    pub(in tree) fn node(&self) -> &Node {
        &self.tree().node
    }

    pub(in tree) fn breadcrumb(&self) -> usize {
        self.tree().node.breadcrumb
    }

    pub(in tree) fn breadcrumb_mut(&mut self) -> &mut usize {
        &mut self.tree_mut().node.breadcrumb
    }

/*    pub(in tree) fn len(&self) -> usize {
        self.tree().len()
    }*/

    pub(in tree) fn path(&self) -> &Path { // for testing
        &self.path
    }

    // Texty Trees //

    pub(in tree) fn is_texty(&self) -> bool {
        self.tree().is_texty()
    }

    pub(in tree) fn text(&self) -> &String {
        self.tree().text()
    }

    /// DANGER: If the text is modified, its parent's node must be updated!
    pub(in tree) unsafe fn text_mut(&mut self) -> &mut String {
        self.tree_mut().text_mut()
    }

    // Foresty Trees //

    pub(in tree) fn is_foresty(&self) -> bool {
        self.tree().is_foresty()
    }
/*
    pub(in tree) fn forest(&self) -> &Vec<Tree<'l>> {
        self.tree().forest()
    }
*/
    /// DANGER: If the forest is modified, its parent's node must be updated!
    pub(in tree) unsafe fn forest_mut(&mut self) -> &mut Vec<Tree<'l>> {
        self.tree_mut().forest_mut()
    }

    pub(in tree) fn num_children(&self) -> usize {
        self.tree().forest().len()
    }

    pub(in tree) fn goto_child(&mut self, i: usize) {
        self.path.push(i);
    }

    // Trees with Parents //
    
    pub(in tree) fn has_parent(&self) -> bool {
        !self.path.is_empty()
    }
    
    pub(in tree) fn index(&self) -> usize {
        *self.path.last().expect("tree_mut: index")
    }

    pub(in tree) fn num_siblings(&self) -> usize {
        self.tree_parent().forest().len()
    }

    pub(in tree) fn goto_parent(&mut self) -> usize {
